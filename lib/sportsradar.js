...
SportsRadar.getShedule = function (message,callback) {
    var url = "http://api.sportradar.us/soccer-t2/eu/matches/2016/03/19/schedule.xml?api_key=2dvtaz23cq6prqjqe8zhahzk";
...
SportsRadar.getMatchDetails = function (matchID,details) {
    "http://api.sportradar.us/soccer-t2/eu/matches/"+matchID+"/summary.xml?api_key=2dvtaz23cq6prqjqe8zhahzk"
    request(url, function (err, response, body) {
         ...
         result["summary"]["matches"][0]["match"][0]["facts"][0]["fact"].forEach(function (value, index) {
             ...
             if(id && description){
                //console.log(id,description);
                 ...
                 var found = false;
                 for (var element in SportsRadar.matchDetails) {
                     if(element.id == obj.id){
                         found = true;
                     }
                 }
                 if(found === false){
                     SportsRadar.matchDetails.push(obj);
                     matches.push(obj);
                 }
